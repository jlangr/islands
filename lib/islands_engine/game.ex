defmodule IslandsEngine.Game do
  ...
  def handle_call({:guess_coordinate, player_key, row, col}, _from, state_data) do
    ...
    with {:ok, rules} <-
           Rules.check(state_data.rules, {:guess_coordinate, player_key}) |> IO.inspect(label: "rules.check"),
         {:ok, coordinate} <-
           Coordinate.new(row, col),
         {hit_or_miss, forested_island, win_status, opponent_board} <-
           Board.guess(opponent_board, coordinate) |> IO.inspect(label: ">>> guess"),
         {:ok, rules} <-
           Rules.check(rules, {:win_check, win_status}) |> IO.inspect(label: "4") 
    do
      state_data
